...
export default function SorteioDogaoPage() {
  ...
  function startSort() {
    ...
    let elapsed = 0;
    const duration = 5 * 1000; // 45 segundos
    ...
    function animate() {
      ...
      const maxDelay = 270;
      let delay =
        minDelay + Math.floor((maxDelay - minDelay) * (elapsed / duration));
  ...
  return (
    <div className="max-w-xl mx-auto py-10 flex flex-col items-center">
      <h1 className="mb-8 text-3xl font-bold text-center">
        Sorteio Premiação de Vendedores
      </h1>
      ...
      <div className="w-full flex flex-col items-center mb-6">
        <div
          className={`h-24 w-full flex items-center justify-center border rounded-lg bg-card shadow ${
      ...
      <Dialog open={!!winner} onOpenChange={(b) => !b && setWinner(null)}>
        <DialogContent className="flex flex-col items-center justify-center py-16 bg-amber-200">
          <h2 className="text-4xl sm:text-6xl font-bold text-amber-700 mb-6">
            🎉 Premiado! 🎉
          </h2>
          <div
          ...
          <Button
            className="mt-10 bg-amber-600 text-white text-lg px-12 py-4"
